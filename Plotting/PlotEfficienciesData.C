...
void MakePlot(const TString xvar, TH1D* htemplate, TCut cuts, const TString plotTitle, int config=1, bool logx = false, bool logy=false, const TString input_directory = "~/muon_trees/trees/SMuData/skims/") 
{ 
  ...
  ch0p0->Add(input_directory+"SingleMuon-FR-0.00-0.00-0.00.root");
  TChain * ch0p5 = new TChain("Tree");
  TChain * ch1p0 = new TChain("Tree");
  if (config==1) { // 2/1
    ch0p5->Add(input_directory+"SingleMuon-FR-0.50-0.00-0.00.root");
    ch1p0->Add(input_directory+"SingleMuon-FR-1.00-0.00-0.00.root");
  } else if (config==2) { // 3/1, 4/1
    ch0p5->Add(input_directory+"SingleMuon-FR-0.00-0.50-0.50.root");
    ch1p0->Add(input_directory+"SingleMuon-FR-0.00-1.00-1.00.root");
  }
  else if (config==3) { //2/1,  3/1, 4/1
    ch0p5->Add(input_directory+"SingleMuon-FR-0.50-0.50-0.50.root");
    ch1p0->Add(input_directory+"SingleMuon-FR-1.00-1.00-1.00.root");
  }

  printf("Entries in chains: %3.0f/%3.0f/%3.0f\n", (double)ch0p0->GetEntries(), (double)ch0p5->GetEntries(), (double)ch1p0->GetEntries());

  
  double nTag0p0 = (double)ch0p0->GetEntries("HLT_IsoMu27_v2&&abs(mu1TLV.Eta())<1.6&&mu1TLV.Pt()>27&&mu1Tight");
  double nTag0p5 = (double)ch0p5->GetEntries("HLT_IsoMu27_v2&&abs(mu1TLV.Eta())<1.6&&mu1TLV.Pt()>27&&mu1Tight");
  double nTag1p0 = (double)ch1p0->GetEntries("HLT_IsoMu27_v2&&abs(mu1TLV.Eta())<1.6&&mu1TLV.Pt()>27&&mu1Tight");
  
  printf("Entries in chains with tag: %3.0f/%3.0f/%3.0f\n", nTag0p0, nTag0p5, nTag1p0);
  ...
  H0p5_pass->SetLineColor(2000);
  H1p0_pass->SetLineColor(2002);
  H0p0_pass->SetMarkerColor(1);
  H0p5_pass->SetMarkerColor(2000);
  H1p0_pass->SetMarkerColor(2002);
  ...
  //  cuts+=("mu1TLV.Pt()>27&&mu1Medium&&abs(mu1TLV.Eta())<1.6");
  ...
  TLegend * leg1 = new TLegend(0.13,0.74,0.43,0.86);
  ...
  thecanvas->cd();
  thecanvas->Print("Plotting/plots/613/"+plotTitle+".pdf");

  TFile* outfile = new TFile("Plotting/hists/613/"+plotTitle+".root", "recreate");
  H0p0_pass->Write();
  H0p5_pass->Write();
  H1p0_pass->Write();
  H0p5_loss->Write();
